...
const AdminEditCoupon = () => {
  ...
  const canEditSplit = coupon?.used_count === 0;
  ...
  }, [form.vendor_share_percentage, form.admin_share_percentage]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only update funding info if coupon hasn't been used
      if (canEditSplit) {
        updateData.funded_by = form.funded_by;
        if (form.funded_by === "vendor") {
          updateData.vendor_id = form.vendor_id;
        }
        if (form.funded_by === "shared") {
          updateData.vendor_share_percentage = parseFloat(form.vendor_share_percentage);
          updateData.admin_share_percentage = parseFloat(form.admin_share_percentage);
        }
      }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-[#7a1c3d] mb-6">Edit Coupon</h1>

      {!canEditSplit && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
          ⚠️ This coupon has been used ({coupon?.used_count} times). Funding type and split percentages cannot be modified.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow-sm border">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input name="code" value={form.code} onChange={handleChange} className="w-full border rounded-lg p-2" required />
          </div>
          ...
          <div>
            ...
            <input name="value" type="number" step="0.01" value={form.value} onChange={handleChange} className="w-full border rounded-lg p-2" required />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Minimum Order Amount</label>
            <input name="min_order_amount" type="number" step="0.01" value={form.min_order_amount} onChange={handleChange} className="w-full border rounded-lg p-2" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Maximum Discount</label>
            <input name="max_discount" type="number" step="0.01" value={form.max_discount} onChange={handleChange} className="w-full border rounded-lg p-2" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Usage Limit</label>
            <input name="usage_limit" type="number" value={form.usage_limit} onChange={handleChange} className="w-full border rounded-lg p-2" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Start Date *</label>
            <input type="date" name="start_date" value={form.start_date} onChange={handleChange} className="w-full border rounded-lg p-2" required />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Expiry Date *</label>
            <input type="date" name="expiry_date" value={form.expiry_date} onChange={handleChange} className="w-full border rounded-lg p-2" required />
          </div>
          ...
          <div>
            ...
            {!canEditSplit && form.funded_by === "shared" && (
              <p className="text-xs text-gray-500 mt-1">Funding type cannot be changed after coupon is used</p>
            )}
          </div>
            ...
            <>
              <div className="col-span-2 bg-blue-50 p-4 rounded-lg space-y-3 border border-blue-200">
                <p className="text-sm font-medium text-blue-800">
                  Split Percentage (Total must be 100%)
                  {!canEditSplit && <span className="text-xs text-gray-500 ml-2">- Read only</span>}
                </p>
                ...
                {totalPercentage !== 100 && canEditSplit && (
